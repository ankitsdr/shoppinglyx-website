...
class ProductView(View):
	def get(self, request):
		...
		essential = Product.objects.filter(category='E')



		if request.user.is_authenticated:
			totalitem = len(Cart.objects.filter(user=request.user))
		return render(request, 'app/home.html', {'top':top, 'bottom':bottom, 'mobiles':mobiles,'laptop':laptop,'essential':essential ,'totalitem':totalitem})
...
def essential(request, data=None):
	totalitem = 0
	if request.user.is_authenticated:
		totalitem = len(Cart.objects.filter(user=request.user))
	if data==None :
			...
			essential = Product.objects.filter(category='E').filter(brand=data)
	elif data == 'below':
			essential = Product.objects.filter(category='E').filter(discounted_price__lt=500)
	elif data == 'above':
			essential = Product.objects.filter(category='E').filter(discounted_price__gt=1000)
	return render(request, 'app/essential.html', {'essential':essential, 'totalitem':totalitem})
...
def top(request, data=None):
	...
	if data==None :
			top = Product.objects.filter(category='TW')
	elif data == 'Spyker' or data == 'Jockey':
			...
			top = Product.objects.filter(category='TW').filter(discounted_price__gt=1500)
